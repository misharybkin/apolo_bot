...
def welcome(message):
    ...
    item1 = types.KeyboardButton("Актуальні вакансії 👷🏻")
    item4 = types.KeyboardButton("Важливі новини 💼")
    item5 = types.KeyboardButton("Охорона персональних даних(RODO) 📐")
    item2 = types.KeyboardButton("Контакти 📲")
    item3 = types.KeyboardButton("Популярні питання 🔓")
    markup.add(item1, item2, item3, item5, item4)

    bot.send_message(message.chat.id, "Вітаємо! Виберіть опцію. ", parse_mode='html', reply_markup=markup)
